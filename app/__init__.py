...
Once human select service, please make appropriate questions from the below common questions for selected service and it must be less than 8
must make candidate answers about quetsion.
...
1. And then, ask to human one by one from start of approrpriate questions and display candidate answers for that question.
2. Human can select one of them or input instead of selection to answer appropriate question.
...
If you asked all the appropriate questions, show human all the inputed values sum up.
display ***END*** at last and then say goodbye
...
# file = open(os.getenv('DOCSDATA'), 'w')
...
memory=ConversationBufferWindowMemory( k=20, return_messages=False)
...
# loader = TextLoader(os.getenv('DOCSDATA'))
